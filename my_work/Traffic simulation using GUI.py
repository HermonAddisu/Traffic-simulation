...
CAR_SIZE = 40  # Fixed square size for all cars
...
class Car:
    def __init__(self, canvas, traffic_light):
        ...
        # Create the square with a fixed size
        self.car = self.canvas.create_rectangle(
            0,
            random.randint(50, HEIGHT - CAR_SIZE - 50),
            CAR_SIZE,
            random.randint(50, HEIGHT - CAR_SIZE - 50) + CAR_SIZE,
            fill=random.choice(["blue", "red", "green", "yellow", "purple"])
        )
